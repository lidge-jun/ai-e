 ...
 * by a person.
 *
 * Agent harnesses run installs and CLIs on the user's behalf and answer prompts
 * from their own logic, which means a consent question would be decided by the
 * agent instead of the account owner. Prompts that act on the user's identity
 * check this and defer: they ask the agent to relay the question instead of
 * answering it.
 *
...
function isAgentDriven(env = process.env) {
  return AGENT_ENV_VARS.some(name => (env[name] || '').trim() !== '');
}

module.exports = { isAgentDriven };
